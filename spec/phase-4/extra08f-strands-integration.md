...
- [x] __main__.py が新実装を使用している
- [x] config.yaml.example が新形式になっている
- [x] 旧実装ファイルが全て削除されている
- [x] 旧テンプレートファイルが全て削除されている
- [x] 旧テストファイルが全て削除されている
- [x] infrastructure/llm/__init__.py が更新されている
- [x] 全テストが通過する

Note: config.yaml はユーザー固有設定のため変更しない（手動移行を想定）
